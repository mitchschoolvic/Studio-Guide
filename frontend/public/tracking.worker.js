...
(() => {
  ...
  var _FaceSolver = class _FaceSolver {
     ...
     */
    static solve(landmarks, imageWidth, imageHeight, fov, headWidthMm, transformMatrix) {
  ...
  var triggerStates = /* @__PURE__ */ new Map();
  function connectCompanion() {
    if (ws2 || !config) return;
  ...
  function sendCompanionCommand(page, bank) {
    if (!ws2 || !isWsConnected) return;
    ws2.send(`BANK-PRESS ${page} ${bank}`);
  }
  function evaluateTriggers(hands, timestamp) {
    ...
    detectors.forEach((d2) => {
      ...
      if (!state) {
        ...
        triggerStates.set(d2.id, state);
      }
      let matchFound = false;
      for (const hand of hands) {
        if (hand.gesture !== d2.gesture) continue;
        const gZone = config.gestureZone;
        if (gZone && gZone.enabled) {
          const box = hand.box;
          const zBox = gZone.box;
          const isInGestureZone = !(box.x > zBox.x + zBox.width || box.x + box.width < zBox.x || box.y > zBox.y + zBox.height || box.y + box.height < zBox.y);
          if (!isInGestureZone) continue;
        }
        matchFound = true;
        break;
      }
      if (matchFound) {
        ...
        const duration = config.gestureZone?.holdDurationMs ?? 150;
        const elapsed = timestamp - state.enterTime;
        if (elapsed >= duration) {
          if (!state.hasTriggered) {
            console.log(`[TrackingWorker] \u27A4 TRIGGERED: ${d2.id} (Duration: ${elapsed}ms)`);
            sendCompanionCommand(d2.page, d2.bank);
            if (ws2 && isWsConnected) {
              const message = JSON.stringify({
                type: "set_variable",
                name: "gesture",
                value: d2.id
              });
              console.log(`[TrackingWorker] Sending Variable: ${d2.id}`);
              ws2.send(message);
            } else {
          ...
          activeTriggers.push(d2.id);
        } else {
          const progress = Math.min(1, elapsed / duration);
          pendingTriggers.push({ id: d2.id, progress });
        }
      } else {
        ...
        if (state.hasTriggered) {
          ...
          if (timeSinceLastSeen < releaseDelay) {
            ...
            console.log(`[TrackingWorker] \u27A4 RELEASED: ${d2.id}`);
            if (ws2 && isWsConnected) {
              const message = JSON.stringify({
                type: "set_variable",
                name: "gesture",
  ...
  async function processFrame(bitmap, timestamp) {
    ...
    try {
      ...
      for (let i2 = 0; i2 < faceResults.faceLandmarks.length; i2++) {
        const landmarks = faceResults.faceLandmarks[i2];
        const matrix = matrices && matrices[i2] ? matrices[i2] : void 0;
